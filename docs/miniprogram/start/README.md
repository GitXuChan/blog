...
直接通过git下载  `Loreal UI`  源代码，并将 `dist` 目录（Lin-UI 组件库）拷贝到自己的项目中。
...
## 使用组件
> 下文会简单介绍一个`Loreal UI` 组件的引入和使用。

1. 配置微信开发者工具

     - 要正确使用 Loreal UI，需将微信开发者工具的 `Es6 转 ES5` 和 `增强编译` 选项勾选上
...
3. 在页面中使用
   ...
   ```
   > 我们建议你在引入 Loreal UI 组件时，都统一命名为 `l-{组件名}` 格式，遵循此规范将使你的项目代码更加清晰

   **至此，Loreal UI 已成功引入至你的项目中了！**
## 自定义配置

> 考虑到开发者在面临不同到项目时，需求和行业的不同。Loreal UI 设计规范上支持一定程度上的样式定制，以满足业务和品牌上多样化的视觉需求。

同时，可以通过对 `components.json` 进行配置，来编译生成相对应的组件。
<br />
为满足自定义的需求，首先我们要去下载 `Loreal UI` 源码。
...
完成以上两步是对 `Loreal UI` 进行自定义配置的基础要求。

### 全局样式更改
> Loreal UI 的样式使用了 Less 作为开发语言，并定义了一系列全局/组件的样式变量，你可以根据需求进行相应调整。
...
其他
更改完成后，在Loreal UI下的根目录里打开终端执行如下所示命令
...
配置完成后，在 `Loreal UI` 下的根目录里打开终端执行
